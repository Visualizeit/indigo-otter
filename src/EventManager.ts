...
export class EventManager {
	...
	setFocused(view: View | null): void {
		this.focused?._eventListeners.forEach(([type, listener]) => {
			if (type === UserEventType.Blur) {
				listener(
					{ bubbles: false, capturable: false, type: UserEventType.Blur },
					this,
				)
			}
		})
		...
		this.focused?._eventListeners.forEach(([type, listener]) => {
			if (type === UserEventType.Focus) {
				listener(
					{ bubbles: false, capturable: false, type: UserEventType.Focus },
					this,
				)
			}
		})
	...
	deliverEvents(root: View): void {
		...
		if (currentlyScrolled) {
			...
			const mouseUp = this.events.find(
				(event) => event.type === UserEventType.MouseUp,
			)
			if (mouseUp) {
	...
	private onMouseMove(node: View, event: MouseEvent): void {
		const deltaX = this.lastPosition
			? event.position.x - this.lastPosition.x
			: 0
		const deltaY = this.lastPosition
			? event.position.y - this.lastPosition.y
			: 0
