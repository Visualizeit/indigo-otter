...
export class WebGPURenderer implements Renderer {
	...
	constructor(
		...
		invariant(
			settings.rectangleBufferSize > 0,
			'Rectangle buffer size must be greater than 0.',
		)
		invariant(
			settings.textBufferSize > 0,
			'Text buffer size must be greater than 0.',
		)
	...
	rectangle(
		...
		this.rectangleData[this.rectangleCount * struct + 12] =
			borderWidth.x * scale
		this.rectangleData[this.rectangleCount * struct + 13] =
			borderWidth.y * scale
		...
		this.rectangleData[this.rectangleCount * struct + 24] =
			clipCorners.x * scale
		this.rectangleData[this.rectangleCount * struct + 25] =
			clipCorners.y * scale
		this.rectangleData[this.rectangleCount * struct + 26] =
			clipCorners.z * scale
		this.rectangleData[this.rectangleCount * struct + 27] =
			clipCorners.w * scale
		this.rectangleData[this.rectangleCount * struct + 28] =
			this.settings.windowWidth * scale
		this.rectangleData[this.rectangleCount * struct + 29] =
			this.settings.windowHeight * scale
	...
	text(
		...
		invariant(
			shape.positions.length === text.length,
			'Shape length does not match text length.',
		)
		...
		for (let i = 0; i < shape.positions.length; i++) {
			...
			this.glyphData[this.glyphCount * struct + 18] =
				this.settings.windowWidth * scale
			this.glyphData[this.glyphCount * struct + 19] =
				this.settings.windowHeight * scale
	...
	render(commandEncoder: GPUCommandEncoder): void {
		...
		for (let k = 0; k < this.drawingIndices.length - 1; k += 2) {
			...
			const rectangles =
				(this.drawingIndices[k + 2] ?? this.rectangleCount) - (r ?? 0)
