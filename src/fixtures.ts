...
export function flexValue() {
	...
	root.add(row)
	const row0 = new View({
		style: { backgroundColor: orange[1], flex: 0, height: 50 },
	})
	row.add(row0)
	const row1 = new View({
		style: { backgroundColor: orange[2], flex: 1, height: 50 },
	})
	row.add(row1)
	const row2 = new View({
		style: { backgroundColor: orange[3], flex: 2, height: 50 },
	})
	row.add(row2)
	const rowFixed = new View({
		style: { backgroundColor: orange[4], height: 50, width: 50 },
	})
	row.add(rowFixed)
	...
	const column0 = new View({
		style: { backgroundColor: orange[5], flex: 0, width: 50 },
	})
	column.add(column0)
	const column1 = new View({
		style: { backgroundColor: orange[6], flex: 1, width: 50 },
	})
	column.add(column1)
	const column2 = new View({
		style: { backgroundColor: orange[7], flex: 2, width: 50 },
	})
	column.add(column2)
...
export function flexShrinkAndGrow() {
	...
	function row(testID?: string) {
		return new View({
			style: { flexDirection: FlexDirection.Row, width: '100%' },
			testID,
		})
	}
...
export function monoFontForm() {
...

	{
		...
		for (const [name, value] of [
			...
			capsule.add(
				new Text(value.toString(), { lookups, style: capsuleTextStyle }),
			)
			wrapper.add(capsule)
...

	{
		...
		const background = new View({
			style: { backgroundColor: '#CCC', padding: 40 },
		})
		...
		headerWrapper.add(
			new Text('Members', { lookups, style: { ...headerStyle } }),
		)
		headerWrapper.add(
