...
import { UserEventType,
	...
	type KeyDownHandler,
	type FocusHandler,
	type BlurHandler } from '../layout/eventTypes'
import {
	Whitespace,
	Position,
	TextAlign,
...
export class Input extends View {
	...
	constructor(
		...
		const baseStyle: ViewStyleProps =
			props.style?.alignSelf !== AlignSelf.Stretch &&
	...
	private onKeyDown(event: KeyboardEvent) {
		...
		const { cursor, mark, value } = updateSelection(
			this.value,
			this.cursor,
			this.mark,
			event,
	...
	private onKeyPress(event: KeyboardEvent) {
		...
		const { cursor, mark, value } = updateText(
			this.value,
			this.cursor,
			this.mark,
			event,
	...
	private onMouseMoveForDragging(event: MouseEvent) {
		...
		text._state.x =
			this._state.x + this._style.paddingLeft - this.horizontalScroll
	}
	...
	private setSelection(startPoint: Vec2, endPoint: Vec2) {
		...
		const fontPadding =
			(atlasGap * fontSize) / this.props.lookups.atlas.fontSize
		...
		for (let i = 0; i < this.textShape.positions.length; i++) {
			...
			if (
				!goingLeft &&
				rectangleX + rectangleWidth > textInputWidth - scrollMargin
			) {
		...
		const lastCharacterWidth =
			(this.textShape.sizes.at(-1)?.x ?? 0) - 2 * fontPadding
		...
		if (
			...
			totalWidth + offsetLeft - lastCharacterWidth / 2
		) {
			if (goingLeft) {
				this.mark = this.textShape.positions.length
			} else {
