...
function paintNode(
	...
	if (node instanceof Text) {
		...
		if (node instanceof View) {
			...
			// Scrollbar.
			if (
				node._state.hasVerticalScrollbar &&
				node._state.hasHorizontalScrollbar
			) {
				ui.rectangle(
			...
			if (node._state.hasHorizontalScrollbar) {
				...
				const scrollTrackSize =
					(node._state.clientWidth / node._state.scrollWidth) * scrollbarSize
				const scrollTrackPosition =
					(node._state.scrollX / node._state.scrollWidth) * scrollbarSize
